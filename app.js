...
function initializeApp() {
    ...
    // محاولة مزامنة البيانات المحلية عند بدء التطبيق
    if (navigator.onLine) {
        setTimeout(syncOfflineData, 2000); // انتظار ثانيتين ثم محاولة المزامنة
    }
}
...
async function createNewSession() {
    ...
    try {
        ...
        // إظهار رسالة مناسبة حسب حالة الاتصال
        if (currentSession.isOffline) {
            showMessage(`تم بدء جلسة جديدة للعميل: ${clientName} (وضع عدم الاتصال)`, 'warning');
        } else {
...
function getOfflineSessions() {
    try {
        const sessions = localStorage.getItem('offlineSessions');
        return sessions ? JSON.parse(sessions) : [];
    } catch (error) {
        console.error('خطأ في قراءة الجلسات المحلية:', error);
        return [];
    }
}
...
function getOfflineBarcodes() {
    try {
        const barcodes = localStorage.getItem('offlineBarcodes');
        return barcodes ? JSON.parse(barcodes) : [];
    } catch (error) {
        console.error('خطأ في قراءة الباركودات المحلية:', error);
        return [];
    }
}

// دالة لمزامنة البيانات عند عودة الاتصال
    ...
    try {
        ...
        for (const session of offlineSessions) {
            try {
                ...
                const cleanSession = {
                    ...
                    total_barcodes: session.total_barcodes || 0,
                    success_count: session.success_count || 0,
                ...
                console.log('⚠️ فشل في مزامنة الجلسة:', session.id, error.message);
            }
        }
        ...
        for (const barcode of offlineBarcodes) {
            try {
                // تنظيف البيانات من الحقول غير المطلوبة
                const cleanBarcode = {
                    session_id: barcode.session_id,
                    barcode_value: barcode.barcode_value,
                    scan_time: barcode.scan_time,
                    is_valid: barcode.is_valid,
                    error_message: barcode.error_message
                };
                ...
                console.log('⚠️ فشل في مزامنة الباركود:', barcode.barcode_value, error.message);
            }
        }
...
function createSampleData() {
    try {
        ...
        // إنشاء جلسات تجريبية مع IDs فريدة
        ...
        // إنشاء باركودات تجريبية مع IDs فريدة
    ...
    try {
        ...
        if (navigator.onLine && supabase) {
            try {
                ...
                console.log('✅ تم حفظ الباركود في قاعدة البيانات:', cleanBarcode);
            } catch (error) {
                console.log('⚠️ فشل في الاتصال بقاعدة البيانات، حفظ محلياً');
                // حفظ محلياً في حالة فشل الاتصال
                saveBarcodeOffline(barcodeData);
            }
        } else {
            console.log('📱 العمل في وضع عدم الاتصال، حفظ محلياً');
            // حفظ محلياً في وضع عدم الاتصال
            saveBarcodeOffline(barcodeData);
        }
